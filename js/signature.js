...
const render = () => {
  ...
  ctx.fillText(inputs.fullname.value.toUpperCase().trim(), 550, 130);
  ctx.font = '46px Source Sans Pro';
  ctx.fillText(inputs.badge.value.toUpperCase().trim(), 550, 190);
  ctx.shadowOffsetY = 3;
  ctx.shadowOffsetx = 3;
  ctx.shadowBlur = 1;
  ctx.font = '34px Source Sans Pro';
  ctx.fontWeight = 'bold';
  ctx.fillText(inputs.line1.value.toUpperCase().trim(), 550, 260);
  ctx.fillText(inputs.line2.value.toUpperCase().trim(), 550, 310);
};
